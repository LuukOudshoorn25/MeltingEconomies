...
#year <- data$year[119:177]
pre2 <- diff(data$mean_pre[119:177])
rad2 <- diff(data$mean_rad[119:177])
tmp2 <- diff(data$mean_tmp[119:177])
GDP_cap2 <- diff(data$NY.GDP.PCAP.KD[119:177])
cpi2 <- diff(as.numeric(data$AG.PRD.CROP.XD[119:177]))
cpi2[is.na(cpi2)] <- 0
